...
import { Welcoming } from "@/constants";
import CustomButton from "@/components/CustomButton";

const Welcome = () => {
  const swiperRef = useRef<Swiper>(null);
  const [activeIndex, setActiveIndex] = useState(0);
  const isLastSlide = activeIndex === Welcoming.length - 1;
  ...
  return (
    <SafeAreaView className="flex h-full justify-between items-center bg-white">
      ...
      >
        <Text className="text-md text-black font-PoppinsBold">Geç</Text>
      </TouchableOpacity>
      <Swiper
        ref={swiperRef}
        loop={false}
        dot={
          <View className="w-[32px] h-[4px] mx-1 bg-[#E2E8D0] rounded-full" />
        }
        activeDot={
      ...
      >
        {Welcoming.map((item) => (
          <View key={item.id} className="flex items-center justify-center">
            ...
            <Text className="text-md font-PoppinsSemiBold text-center text-[#858585] mx-5 mt-3">
              {item.description}
            </Text>
          </View>
        ))}
